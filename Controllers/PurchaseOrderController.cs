...
namespace StoreManagement_Project.Controllers
{
    public class PurchaseOrderController : Controller
    {
        ...
        public IActionResult Create()
        {
            ...
            //    .Include(i => i.Unit)
            //    .ToList();
            ViewBag.Items = _context.PurchaseRequisitionItems
                    .Include(pri => pri.Item)
                    .Include(pri => pri.Unit)
                    .Select(pri => pri.Item)
                    .Distinct()
                    .ToList();
        ...
        public async Task<IActionResult> Create(PurchaseOrder purchaseOrder)
        {
            try
            {
                ...
                // ✅ Sanitize Remarks to remove HTML and links
                string remarks = purchaseOrder.Remarks ?? "";
                remarks = Regex.Replace(remarks, @"<a[^>]*>(.*?)<\/a>", "", RegexOptions.IgnoreCase);         // Remove <a> tags
                remarks = Regex.Replace(remarks, @"(http[s]?://\S+|www\.\S+)", "", RegexOptions.IgnoreCase); // Remove URLs
                remarks = Regex.Replace(remarks, "<.*?>", string.Empty);                                     // Remove all HTML
                ...
                if (ModelState.IsValid)
                {
                    ...
                    // Set POManualNumber to PONo if not provided
                    if (string.IsNullOrWhiteSpace(purchaseOrder.POManualNumber))
                    {
                        purchaseOrder.POManualNumber = purchaseOrder.PONo;
                    }
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            //  Manually fetch the manual PO number from another source (e.g., a separate table)
            var manualNo = await _context.PurchaseOrders
                .Where(p => p.PurchaseOrderId == id)
                .Select(p => p.POManualNumber)
                .FirstOrDefaultAsync();

            // 🔸 Assign the value to the NotMapped property
            purchaseOrder.POManualNumber = manualNo;
        ...
        public async Task<IActionResult> Edit(PurchaseOrder purchaseOrder)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    existingPO.Currency = await _context.Currencies.FindAsync(purchaseOrder.CurrencyId);
